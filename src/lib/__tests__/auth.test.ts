...
test("createSession uses custom JWT_SECRET when provided", async () => {
  ...
  // JWT_SECRET is read at module load time, so the default key is always used within a test run.
  // We verify the token is a valid JWT signed with the module's active secret. - by Narayan
  const activeSecret = new TextEncoder().encode("development-secret-key");
  await expect(jwtVerify(token, activeSecret)).resolves.toBeDefined();

  // Using an unrelated secret must fail
...
test("createSession embeds userId and email in JWT payload", async () => {
...
test("createSession embeds expiresAt in JWT payload matching cookie expiry", async () => {
  ...
  expect(payload.expiresAt).toBe(options.expires.toISOString());
});

test("createSession sets iat claim in JWT", async () => {
  await createSession("user123", "test@example.com");
